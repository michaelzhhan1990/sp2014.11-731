...
def extract_features_n(hyp, ref,n,box):
    ...
    while i<=n:
        ...
        if len(hwords)<i or len(rwords)<i:
...
def all(hyp, ref):
     ...
     feature={}
     while n<=6:
         st=extract_features_n(hyp,ref,n)
         ss=extract_features_n(hyp,hyp,n)
         tt=extract_features_n(ref,ref,n)
         feature[str(n)+'-gram']=st/math.sqrt(ss*tt)
         n+=1
     """
     n=10
     features={}
